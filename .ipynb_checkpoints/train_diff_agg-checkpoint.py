...
def compute_metrics(p):
    ...
    with multiprocessing.Manager() as manager:
        ...
        lock = manager.Lock()
        processes = []
        for idx, (l, p) in enumerate(zip(true_labels, true_predictions_bin)):
            # 為每一組標籤和預測創建一個新的進程來計算 disorder
            process = multiprocessing.Process(target=get_observed_disorder_in_process, args=(l, p, return_dict, idx))
        ...
        # 收集所有 disorder 的結果
        disorder = [return_dict[idx] for idx in range(len(true_labels))]
