...
def evaluate_a_pair_highlight(pred, truth, pred_threshold=0.5) -> dict:
    ...
    assert len(pred["pred_prob"]) == len(truth["highlight_probs"]), f"Length mismatch: {len(pred['pred_prob'])} vs {len(truth['highlight_probs'])}"
    ...
    topr_pred_index = sorted(range(len(pred["pred_prob"])), key=lambda i: pred["pred_prob"][i], reverse=True)[:r_truth]
    r_precision = len(set(truth_index) & set(topr_pred_index)) / r_truth if r_truth > 0 else 0

    # calculate the correlation 
    if np.std(truth["highlight_probs"]) != 0:
        correlation = np.corrcoef(truth["highlight_probs"], pred["pred_prob"])[0, 1] if r_truth > 0 else 0
